...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Login
  // ✅ inside AuthProvider.tsx - in login()
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    // 🔸 Wait a short moment for Supabase to persist session
    ...
    // ✅ Set user and refresh router to sync UI
    setUser(userData)
  ...
  // Bullet-proof logout
  // ✅ logout() - ensures immediate redirect before re-render
const logout = async () => {
  try {
    ...
    // Remove tokens/caches
    localStorage.removeItem("supabase.auth.token");
    sessionStorage.clear();

    // Force a short delay so Supabase completes cleanup
    await new Promise((r) => setTimeout(r, 200));
  } catch (err) {
    console.error("Logout failed:", err);
  } finally {
    // 🚀 Immediately push user to login before component re-renders
    router.push("/");
    router.refresh();
  }
};
